...
class MockGetCompleteModuleUseCase extends Mock
    implements GetCompleteModuleUseCase {}

void main() {
  late ModuleInfo mockModuleInfo;
  late MockNavigatorObserver mockNavigatorObserver;
  late MockGetCompleteModuleUseCase mockGetCompleteModuleUseCase;
  ...
  setUp(() {
    ...
    mockGetCompleteModuleUseCase = MockGetCompleteModuleUseCase();
    ...
    // Mock the use case to return the test module
    when(() => mockGetCompleteModuleUseCase.execute(any()))
  ...
  testWidgets('ModuleDetailPage shows module properties',
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          getCompleteModuleUseCaseProvider
              .overrideWithValue(mockGetCompleteModuleUseCase),
        ],
        child: MaterialApp(
    ...
    // Wait for postFrameCallback + async loadCompleteModule
    await tester.pump(); // First frame (triggers postFrameCallback)
    await tester.pump(); // Process postFrameCallback
    await tester.pump(const Duration(milliseconds: 500)); // Wait for async loadCompleteModule
    await tester.pump(); // Rebuild after setState

    // Verify the page renders and module title is shown in the AppBar
    expect(find.textContaining('Test Module'), findsWidgets);
  });
