...
final observationsProvider = StateNotifierProvider.family<
    ObservationsViewModel, AsyncValue<List<Observation>>, int>((ref, visitId) {
  final getObservationsByVisitIdUseCase = ref.watch(getObservationsByVisitIdUseCaseProvider);
...
class ObservationsViewModel extends StateNotifier<AsyncValue<List<Observation>>> {
  ...
  Future<void> loadObservations() async {
    ...
    try {
      ...
      final observations = await _getObservationsByVisitIdUseCase.execute(_visitId);
      if (_mounted) {
  ...
  Future<int> createObservation(Map<String, dynamic> formData) async {
    try {
      ...
      final newObservationId = await _createObservationUseCase.execute(observation);
  ...
  Future<bool> updateObservation(Map<String, dynamic> formData, int observationId) async {
    try {
      ...
      final updatedObservation = Observation(
        ...
        idBaseVisit: _visitId,
        cdNom: formData['cd_nom'] is int ? formData['cd_nom'] : existingObservation.cdNom,
        comments: formData['comments']?.toString() ?? existingObservation.comments,
      ...
      final success = await _updateObservationUseCase.execute(updatedObservation);
  ...
  Map<String, dynamic> _extractObservationSpecificData(Map<String, dynamic> formData) {
    ...
    formData.forEach((key, value) {
      if (!standardFields.contains(key) && value != null) {
        ...
        } else if (key.toLowerCase().contains('time') && !key.toLowerCase().contains('date') && value is String) {
